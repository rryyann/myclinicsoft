...
class Queings extends Secure {
	...
	function move_in($id){
		...
		$data = array(
			'que_id'  		=> $this->Queing->last_row($this->license_id)->que_id + 1, //unique id
			'que_date'		=> date('Y-m-d'),
	...
	function move_out($rowid, $status) { 
        ...
        if($this->Queing->delete($rowid, $status, $this->license_id)){
	...
	function clear_all(){
		...
		if($this->Queing->delete_list($this->license_id)){
	...
	function get_counts(){
		$counts = 0;
		$counts = $this->Queing->count_all($this->license_id);
